...
from backend.practices.practices_classes.practice_global_settings.individual_practice_classes.reading_practice_global_settings import ReadingPracticeGlobalSettings
...
class ReadingPracticeTopicGenerator(PracticeTopicGenerator, ReadingPracticeGlobalSettings, DialogueSetting):
    def __init__(self, user: User):
        # ------------------- Init parent classes -------------------
        PracticeTopicGenerator.__init__(self, user)
        ReadingPracticeGlobalSettings.__init__(self)
    ...
    def set_user_settings_from_user(self, user: User):
        ...
        self.set_difficult_words(user.reading_settings.get_difficult_words())
        self.set_final_writing_question(user.reading_settings.get_final_writing_question())
        self.set_paragraphs_number(user.reading_settings.get_paragraphs_number())
        self.set_dialogues_number(user.reading_settings.get_dialogues_number())

        # Difficulty level specific for reading
        self.set_difficulty_level(user.reading_settings.get_difficulty_level())
